...
import { BookStatus, BookStatusName } from '../constants/bookStatus';
...
export default function BookDetailPage() {
  ...
  const { data: book, isLoading, error } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return status === BookStatus.Uploaded || status === BookStatus.Processing ? 3000 : false;
    },
  });
    ...
    <div>
      ...
      {book.chapters.length === 0 ? (
        <p className="empty-state">
          {book.status === BookStatus.Processing ? 'Processing book... chapters will appear shortly.' : 'No chapters detected yet.'}
        </p>
      ) : (
